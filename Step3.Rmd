...
knitr::opts_chunk$set(echo = FALSE) # echo = FALSE means R will not print the code along with the results. 
...
dir_output_csvs = './Results/Test/JBER/' 
...
stackX <- read_stars(files_s1f1[[1]]) # creates raster stack for first variable in s1f1 list (using s1f1 because has fewer files than baseline)

dims <- dim(stackX) # get dimensions for example stack

random_raster <- as.numeric(sample(1:dims[3], 1)) # get a random raster layer from StackX to use as an example

rx <- stackX[, random_raster,]

rx <- slice(stackX[[random_raster]]
stackXstars <- st_as_stars(stackX)
# AFB 
...
afb <- vect(afbSF)
afb <- aggregate(afb) # dissolves boundaries in shp
...
rxCrop <- terra::crop(rx, afb, mask = TRUE) # snap = "out" shows all cells that overlap AFB polygon

rxCrop2 <- extend(rxCrop, afb)
...
Next, we will check the size of the AFB to see whether we should use its centroid or spatial extent to extract the data we need. If the AFB fits within a single NetCDF cell, it will be much faster to extract data using a single point (the centroid). The script will assume that if the AFB is smaller than the NetCDF cell, you will want to use the centroid, and if it is larger, you will want to use the spatial extent. You can override this by changing the 'extract_obj' object to 'afb' (spatial extent) or 'cell' (centroid).     

INTERPRETING OUTPUT: If the output of the following code chunk says 'numeric', the script will use the centroid. If it says 'spatVector', it will use the spatial object (afb).

*NOTE TO SELF: CONSIDER CONDITION IF AFB OVERLAPS MULTIPLE CELLS RATHER THAN SIZE)*
...
The next chunk reorganizes the extracted data into a more workable dataframe ("tibble", the the tidyverse). The output is equivalent to the 'AllValuesDataframe' from the original Step3 script. 
...
# PART FIVE: ADD DERIVED VARIABLES
...
# If for some reason the AllDays object does not work, you can use the script './Code/Misc/MonthSum.R' to run the MonthSum analyses using the csv output files one-by-one. This will be the best option for MACA inputs, since this script cannot yet accommodate MACA models. 
