...
*When loading .RData files, you need to re-run the raster (variable name = rx)* The following code is for MAC ONLY:

```{r MAC-rerun-rasters}

model_dir <- paste("/Volumes/wcnr-network/RStor/mindyc/afccm/Climate Modeling/Data/", model, sep = "") # Directory for model data

# Create files_s1f1_MAC for precip only

dir = paste(model_dir, scenario1, sep = '/')
fileNames = list.files(dir, pattern = 
                        "hurs_d", full.names = TRUE)
fileNames = str_remove_all(fileNames, pattern = ".aux.xml")
fileNames = fileNames[!duplicated(fileNames)]
dt = data.table(fileNames, result = grepl(future1_yrs, fileNames))
dt2 = dplyr::filter(dt, result == TRUE)
files_s1f1_MAC = dt2$fileNames

# Create Rasterstack

stackX <- rast(files_s1f1_MAC)
...
#saveRDS - save this if necessary to call rx object without re-running raster part of script

## There might be a quicker way to get an example raster without creating a stack

#fileNumber = as.numeric(sample(1:length(files_s1f1_MAC), 1))

#rx <- rast(files_s1f1_MAC[6])

```

```{r raw-ncdf data}

x <- rast(files_s1f1_MAC[1])
...
rawDataDir <- paste0('/Volumes/wcnr-network/RStor/mindyc/afccm/Climate Modeling/Workspace Images/',installation) # Mac notation
...
#list.files(path = paste0('/Volumes/wcnr-network/RStor/mindyc/afccm/Climate Modeling/Workspace Images/',installation), full.names = TRUE)
