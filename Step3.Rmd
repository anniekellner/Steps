...
rasterStack_filename <- paste0("rx_",image_filename,".Rds")
...
#localDir <- paste0('./Data/',installation)

if (!dir.exists(localDir)){
  dir.create(localDir)}

# Save workspace image to N: drive and local data folder

save.image(file = paste0(rawDataDir,"\\",rawData_filename)) # save to N: drive
save.image(file = paste0(localDir,"/",rawData_filename))

# Save raster stacks

saveRDS(rx, file = rasterStack_filename)
```
...
# This chunk is unnecessary if the remainder of the script is run immediately following data extraction, so it is commented out. *If the base is being re-run from the saved image (.RData file), remove the '#' and run the chunk. After doing do, please remember to comment out again once the chunk has been run*  
...
source('./Code/Plots/Bar_Charts/Comparison_Bar_Charts.R')
