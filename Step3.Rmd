...
dir_installation_boundaries <- ifelse(Navy == FALSE, # site boundary (shapefile used for clipping) # Annie Mac
                              "/Volumes/mindyc/afccm/AF_CIP_ENV_Data_Phase3/Installation_Boundaries",
                              "/Volumes/mindyc/afccm/AF_CIP_ENV_Data_Phase3/Installation_Boundaries/NAVY/")
...
# model_dir <- paste("N:\\RStor\\mindyc\\afccm\\Climate Modeling\\Data\\", model, sep = "") # PC location
model_dir <- paste("/Volumes/mindyc/afccm/Climate Modeling/Data/", model, sep = "") # Mac location
...
for(i in 1:length(nested_tibbles)){
  ...
  tbl = tibble(lat = centroid[2], # will need to be changed when spatial objects are used for extraction
               lon = centroid[1],
               date = pluck(sf_combo, 1, "date")) # pulls date column from first sublist
  sf_tbl = sf %>%
    as_tibble(sf, .name_repair = "unique") %>%
...
csv_file <- './Results/Test/CNMI_Proxy_v1_historical_1986 - 2015_AllDays.csv'

csv <- read_csv(csv_file, show_col_types = FALSE) # I think this should be redone such that the code uses the dataframes created above rather than the resultant .csv file. When you run another model, you can save the environment in an .RData file and I can rewrite this part so you don't have to manually enter directories for the .csv files

csv <- select(csv, -1) # for testing, because row id was included in prior script. Changed to write_csv on 3/8/23 above so hopefully this line won't be necessary. Can comment out or delete entirely when we use R objects instead of .csv products for this chunk
