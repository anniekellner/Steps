...
####################################################################################################
###                PART ONE: USER SETTINGS I - SELECT OR ADD INSTALLATION NAME                   ###
####################################################################################################
...
cat(paste("The official name for the installation you are running is",official_name,".","The abbreviated name for the installation shapefile is", shp,"and the general acronym associated with the installation is", base_abbrev,"."))
...
##################################################################################################################
###           PART TWO: RUN ANALYSES OF HISTORICAL DATA                                                       ###
##################################################################################################################

Typically, the analysis portion would follow the user entries. However, because the historical analysis uses a different dataset from the rest of the script, we are running it before adding settings that pertain to later stages of the analysis.

##    ----        ANALYSIS OF NOAA HISTORICAL DATA    ---       ##
...
*If only one file is listed or you know which .csv you need, please copy-paste the filename from above WITHOUT THE ".csv" SUFFIX into the 'weather_station object (include the quotation marks)*

```{r NOAA}

weather_station <- "Eglin_NicevilleOnly"

# Read in data

noaa_dataFilepath <- paste0(NOAA_dir, weather_station,".csv")

noaa <- read.csv(file = noaa_dataFilepath) # delete this once script is set
...
Similar to what you did above, now *enter the number between the brackets ([]) that corresponds with the model of interest.*
...
*1. If you re-opened RStudio prior to running the script from the workspace image, make sure to run the first two chunks so the settings and libraries are loaded.* 

*2. Remove the #'s that precede the code for the following two chunks.*

*3. Enter the installation of interest where prompted in the first code chunk*

*4. Run the first chunk (get-saved-files)* This will give you the pathnames to enter for the second chunk, which will load the files. 

*5. In the second chunk (load-saved-files), copy-paste the full paths to the .RData and .Rds files where instructed* 

*6. Run the second chunk* You will receive an error about the rx file, but you can ignore it. The second line of code takes care of it. 

*7. Comment out (#) the code for both chunks after running*

*8. Proceed to the portion of the script that starts with "AVDF data frame"*
...
#load("N:/RStor/CEMML/ClimateChange/CC_Modeling/Data/Workspace Images/Eglin_Niceville/Eglin_Niceville_EC-Earth3-Veg-LR_20250123.RData") # INSERT COPY-PASTED PATH TO .RDATA FILE
...
#rm(noaa_monthSum) ######## THIS NEEDS TO BE DEALT WITH - THROWS AN ERROR. THE NOAA DATA NEEDS TO BE REMOVED PRIOR TO RUNNING THE REST OF THE SCRIPT
