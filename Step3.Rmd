...
model_dir <- paste('/Volumes/wcnr-network/RStor/mindyc/afccm/Climate Modeling/Data/', model, sep = "") # Mac location
 # See if this works:

# model_dir <- paste0('\\RStor\\mindyc\\afccm\\Climate Modeling\\Data',"/",model, sep = "") # nope

####### Attempting to use a UNC address for cross-platform compatibility. So far not working. 
# model_dir <- paste('smb:\\acns.colostate.edu\\wcnr-network\\RStor\\mindyc\\afccm\\Climate Modeling\\Data\\', model, sep = "")
# model_dir <- '//mindyc/afccm/Climate Modeling/Data/'


# Uniform naming convention (UNC) for cross-platform compatibility
# model_dir <- file.path(model_dir) # can possibly ensure that directory is a filepath on all OS's
...
# Add Fahrenheit values to final dataframe created by WL_Adapted script
...
# Vertical tags
  ...
  ticks <- data.frame(
    x = seq(0, 12),
    ymin = 25, # changed to reflect Fahrenheit values (originally ymin = -3, ymax = 0)
    ymax = 32
  )

#### START HERE ##########
  # assigned wandlplot from original script - finish Fahrenheit plot using this code
  # can probably delete Fahrenheit-specific function, but make sure nothing critical is in there
 ...
 wandlplotF <- wlCplot +
  ...
  wandlplotF <- wandlplotF +
    ...
    ggplot2::theme(
      ...
      axis.text.y.left = element_text(colour = "red", size = 10),
      ...
      axis.text.y.right = element_text(colour = "blue", size = 10)
    )
  
  
  return(wandlplot)
}
#source('./Code/Functions/WL_Fahrenheit.R')

wlFplot <- ggclimat_walter_lieth(wlC, 
                      est = "Malmstron AFB", 
                      per = "1985 - 2014", 
                      mlab = "en", 
                      pcol = "blue", 
  ...
  # Helper for ticks - check if this is needed
