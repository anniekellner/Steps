...
csv <- read_csv('./Results/Test/CNMI_Proxy_v1_historical_1986 - 2015_AllDays.csv', show_col_types = FALSE) # 
csv <- select(csv, -1) # for testing, because row id was included in prior script. Changed to write_csv on 3/8/23 above so hopefully this line won't be necessary.
...
YrAverage <- rbind(csv3, c("YrAverage", colMeans(csv3[,2:ncol(csv3)]))) # this works but converts columns to characters 
...
YrTotals <- YrTotals %>%
  mutate_at(c(2:ncol(YrAverage)), as.numeric) # in case the results in the .csv should be numeric
