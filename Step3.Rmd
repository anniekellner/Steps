...
afbSF <- sf::st_as_sf(afbSF, coords = c("longitude", "latitude"), crs = st_crs(rx)) 
...
rxCrop <- terra::crop(rx2, afb)

#st_crs(afbSF) = crs(rx) # just in case
...
for(i in 1:length(avdf)){
  ...
  csv_years = if (str_detect(names(avdf[i]), "f1")){
    ...
    write.csv(csv, file = paste(dir_output_csvs,csv_fileName,".csv", sep = ""))
}
```
