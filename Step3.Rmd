...
official_name <- official_base_names_for_plots[1] # These names serve as titles for the climatograph and Walter-Lieth plots
...
*Enter the scenarios of interest below, as well as the way you'd like the scenarios to appear in plots.* You only need to input values between the quotation marks (""). The vectors will populate automatically.
...
scenario1_plot_description <- "SSP2-4.5"
scenario2_plot_description <- "SSP5-8.5"

scenarios <- c(baseline, scenario1, scenario2)
scenarios_for_plots <- c("Historical", scenario1_plot_description, scenario2_plot_description)
...
per_years = case_when( # per is 'period' and is an argument in the ggclimat_walter_lieth() function
...
per_scenario = if (str_detect(names(monthSum[i]), "baseline")){
    scenarios_for_plots[1]
  } else if(str_detect(names(monthSum[i]),"s1")) {
    scenarios_for_plots[2]
  } else {
    scenarios_for_plots[3]
  }
