...
First, let's *select which base (AFB, etc.) you wish to run.* You will need to select the name that corresponds with the correct shapefile as well as the official name of the installation for use with plot titles.  

To choose from an existing list, you can either a) enter "abbrev_name_shp" and "official_base_names" (without the quotation marks) into the console, and select the appropriate number that corresponds with the installation of interest. You can then enter that number between the brackets ([]) below. 

Alternatively (b), you can highlight the following code and run it (ctrl + enter) and do the same thing.

`abbrev_name_shp`
`official_base_names`

For example, in the code chunk below ({enter-base-name}) Homestead_ARB would be abbrev_name_shp[1], and NAVBASE San Diego would be official_base_names[2].
...
weather_station <- "JBER - Anchorage International"
...
# ADD NOAA analyses here bc environment gets messy. Easier to just do now. 
