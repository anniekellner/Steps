...
installation <- installation_names[28] 

official_name <- official_base_names_for_plots[12] # These names serve as titles for the climatograph and Walter-Lieth plots
...
```writeRaster(rx, filename = '/path/to/dir')```
...
save.image(file = paste0(directory,image_filename,".RData"))
...
The two chunks that follow are commented out, assuming the chunks are being run while the global environment still contains all defined objects from the first part of the script (i.e., through data extraction). If not, read the *orange* chunk below and follow instructions.

*If the base is being re-run from the saved image (.RData file) on the N: drive, remove the '#' to un-comment out the code chunks and run.* 

*The first chunk will retrieve the filenames to enter for the second chunk, which will load the files. If you are re-running the installation from an empty environment, you will need to re-enter the installation name first (you don't need to run from the top of the script; there is a portion within the first chunk that allows you to do this).* 

*Also, if you're running within a new RStudio session, don't forget to run the {r setup} and {r load-libraries} chunks at the top, to ensure the required packages are installed. You will get errors if you forget to do this.*
...
*Copy-paste the paths printed in the output after the `load()` and `readRDS()` commands to load the data. These will load the .RData file (global environment) and raster stack, respectively). The code below is sample code from Cape Canaveral.*

```{r load-saved-environment}
...
The following chunk produces a summary table comparing historical averages/totals of select variables to near- and far-term projected values
...
Inputs: monthSum list, diffHist list
Outputs: .png files into ./results_folder/Plots/Bar_Charts
...
source('./Code/Plots/Hist_Bar_Charts.R')
source('./Code/Plots/Comparison_Bar_Charts.R')
