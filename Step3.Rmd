...
for(i in 1:length(monthSum)){
  ...
  csv = bind_rows(monthAvg, YrAverage, YrTotals)
  ...
  NAs_Totals <- csv %>% # YrTotals
    ...
    mutate(across(.cols = contains("Abs"), ~na_if(.,.))) %>%
    mutate(across(.cols = 14:ncol(csv), ~na_if(.,.))) # assuming these columns stay in the same order
  ...
  # Filename
    csv_scenario = if (all$year[1] < as.integer(year(now()))){
    "historical"
  ...
  csv_years = if (str_detect(names(AllDays[i]), "f1")){
    ...
    paste(first(year(AllDays$results_s1f2$date)), "-", last(year(AllDays$results_s1f1$date)))
  } else {
