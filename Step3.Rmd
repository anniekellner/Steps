...
#saveRDS(results_s1f1, file = './Results/s1f1_list.Rds')

system.time( # 10.8 minutes AK home desktop
...
# Need to pair dates with extracted data before removing! Do this in the loop!

rm(list = ls(pattern = "^files")) # remove lists from the environment that aren't results
rm(list = ls(pattern = "^dates"))
```
...
tibbles <- as_tibble(lists) # tibbles are tidy objects

for(i in 1:length(tibbles))
  tbl_nested = tibbles[i]
  tbl <- unnest(tbl_nested) %>%
    mutate(lat = centroid[2]) %>%
    mutate(lon = centroid[1]) %>%
    mutate(date = )
  
  
  
create_allvaluesdf <- function(list){
