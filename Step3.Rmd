...
AFB_Name <- "CNMI_Proxy_v1" # Can create a vector similar to 'models' above if desired
...
rm(list=setdiff(ls(), c("AllDays", "AFB_Name", "model", "scenario1", "scenario2", "dir_output_csvs")))
...
csv_scenario = if (str_detect(names(monthSum[i]), "baseline")){
  ...
  csv_years = if (str_detect(names(monthSum[i]), "f1")){
    ...
    write_csv(monthSum[[i]], file = paste(dir_output_csvs,csv_fileName,".csv",sep = ""))
}
```

```{r remove-objects}

rm(list=setdiff(ls(), c("monthSum", "AFB_Name", "model", "scenario1", "scenario2", "dir_output_csvs")))
...
# This chunk uses the monthSum dataframe 
...
histVals <- hist %>%
  ...
  #replace(is.na(.), 9999) %>%
  mutate(across(where(as.numeric(is.na(.), 9999))))
  mutate(across(everything(), ~replace_na(9999)))
  mutate(across(everything(), ~replace_na(., as.numeric(9999)))) %>%
...
for(i in 2:length(monthSum)){ # starts with 2 because [[1]] is the baseline (historical) to be subtracted
  df = monthSum[[i]]
  df = select(df, where(as.numeric), na.rm = FALSE)
  diffHist = map2(hist, df, ~df-hist)
...
hist = monthSum[[1]][,2:ncol(monthSum[[1]])]
hist = monthSum[[1]][,2:ncol(monthSum[[1]])]
df = monthSum[[i]][,2:ncol(monthSum[[i]])]
DiffHist = map2(hist, df, ~.df-.hist)
DiffHist = map2(hist, df, ~df-hist)
head(DiffHist)
i = 2
