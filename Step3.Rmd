...
for(i in 1:length(files_s1f2)){
  ...
  #times_s1f2 = time(r)
  vals = r[extract_obj]
  results_s1f2[[i]] = pivot_longer(results_s1f2[[i]],
                           cols = everything(),
                           names_to = "Layer",
                           names_prefix = variables[i],
                           values_to = variables[i],
...
for(i in 1:length()) # figure out way to put baseline, s1f1, etc. into a vector
...
csv <- s1f1 %>%
  ...
  mutate(TMaxF = case_when(
      mean(tmax > 200) ~ RasterUnitConvert(tmax, "KtoF"), # Because only values in Kelvin would be > 200
      RasterUnitConvert(tmax, "CtoF"))) %>% # assuming if temp units are not Kelvin they will be Celsius
  mutate(TmaxC = case_when(
    mean(tmax > 200) ~ RasterUnitConvert(tmax, "KtoC"),
    TRUE ~ tmax)) %>% # assuming if temp units are not Kelvin they are Celsius
  mutate(TMinF = case_when(
    mean(tmin > 200) ~ RasterUnitConvert(tmin, "KtoF"),
    RasterUnitConvert(tmin, "CtoF"))) %>%
  mutate(TminC = case_when(
    mean(tmin > 200) ~ RasterUnitConvert(tmin, "KtoC"),
...
avdf_baseline_test <- avdf_baseline %>%
