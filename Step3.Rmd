...
conflicts_prefer(
  lubridate::month(),# this ensures that the correct package is used when multiple packages contain the same function
  dplyr::last()
)
```
...
dir_output_csvs = './Results/Test/CMNI_Proxy_v1/' 
...
The next chunk creates MonthSum.csv files. If for some reason the AllDays object does not work, you can use the script './Code/Misc/MonthSum.R' to run the MonthSum analyses using the csv output files one-by-one. This will be the best option for MACA inputs, since this script cannot yet accommodate MACA models.
