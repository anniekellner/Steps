...
First, let's select which base (AFB) and which of the `models` you'd like to run. To choose from a list, highlight the word `AFB_name` and/or `models` and click 'Run' at the top right (or use the shortcut *Ctrl + Enter*). The available models correspond to numbers in the vector (e.g., HADGEM2-ES is [1]). 
...
for(i in 1:length(avdf)){
  ...
  csv = csv %>%
    ...
    dplyr::select(., lat, lon, date, PPT_in, PPT_mm, TMaxF, TMinF, TMeanF, GDDF, hotdays, colddays, wetdays, drydays, ftdays)
