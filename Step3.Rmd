...
dat_long_end$p_mesIN <- dat_long$p_mes/25.4
dat_long_end$tm_maxF <- (dat_long$tm_max)*(9/5) + 32
dat_long_end$tm_minF <- (dat_long$tm_min)*(9/5) + 32
dat_long_end$ta_minF <- (dat_long$ta_min)*(9/5) + 32

  #dat_long_end$tmF <- (dat_long[[7]] + dat_long[[8]]) / 2 # Fahr Tmax and Tmin

  # Subtitles
  sub <-
    paste(round(mean(dat_long_end[dat_long_end$interpolate == FALSE, ]$tmF), 1),
          "F        ",
          prettyNum(
            round(sum(
              dat_long_end[dat_long_end$interpolate == FALSE, ]$p_mesIN
            )),
            big.mark = ","
          ),
          " in",
