...
official_name <- official_base_names[37] # These names serve as titles for the climograph and Walter-Lieth plots
...
variables <- c("pr_day", "tasmax", "tasmin", "hurs_d", "huss_d", "sfcWin") # an example of how you would choose among the variables if desired
...
dir_installation_boundaries <- case_when(
  Navy == FALSE ~ "N:/RStor/CEMML/ClimateChange/CC_Modeling/Data/Spatial/Installation_Boundaries",
  Navy == TRUE & year2026 == FALSE ~
    "N:/RStor/CEMML/ClimateChange/CC_Modeling/Data/Spatial/Installation_Boundaries/NAVY",
  Navy == TRUE & year2026 == TRUE ~
    "N:/RStor/CEMML/ClimateChange/CC_Modeling/Data/Spatial/Installation_Boundaries/2026 Targets")
...
weather_station <- shp # this is usually the shp name
...
baseGeo_dir <- (paste(dir_installation_boundaries, shp, sep = '/'))
baseGeo_filename <- paste(afb_dir, '.shp', sep = "")

baseSF <- st_read(baseGeo_filename) # Because plotting package (tmap) doesn't take terra vectors
elev <- baseSF$Elev_Mean # add elevation data to sf object
...
baseSF <- if (xmax(rx) == 360 & xmax(base_spatVect) < 0) {st_shift_longitude(baseSF)} else baseSF # adjustment in case ncdf lon scale is 0-360

base_spatVect <- vect(baseSF)
base_spatVect <- terra::aggregate(base_spatVect) # dissolves extraneous boundaries within shp

# Crop single raster to AFB shape

rxCrop <- terra::crop(rx, base_spatVect, snap = "out") # snap = "out" shows all cells that overlap AFB polygon
...
Next, we will check the size of the installation to see whether we should use its centroid or spatial extent to extract the data we need. If the installation fits within a single NetCDF cell, it will be much faster to extract data using a single point (the centroid). The chunk below calculates the area of the ncdf cell as well as the area of the installation itself. The output tells you which to use.

**note: as of 2026-02-06, we only use the centroid for data extraction**. Most of this code is left here in case we want to use the base in its entirety in the future. Most of the time, the installation is smaller than the ncdf cell, so you will not have to change anything. In the offchance the installation is larger than the ncdf cell, you will find instructions below for how to switch the object back to the centroid.    
...
rx_cellSize <- expanse(rxCrop, unit = "km") # size of the cropped raster, returned as a spatRaster
#rx_cellSize <- rx_cellSize$area # returns numeric value

# How big is the installation?

base_area <- expanse(base_spatVect, unit = "km") # for vector data, a single value (area) is returned


centroid <- geom(centroids(base_spatVect, inside = TRUE)) # inside = TRUE guarantees that the centroid is contained within the boundaries of the installation (in some cases, depending on the shape of the AFB, the true centroid may be outside)
...
baseExt <- ext(base_spatVect) # get geographic extent. This is not currently used.


# add an if_else statement to tell R whether to extract raster values from matrix or shapefile

extract_obj <- ifelse(base_area < rx_cellSize, cell, baseExt) 

 

cat("The object used for data extraction is", 
    ifelse(class(extract_obj) == "numeric", "the centroid", "the installation's spatial extent")) 
...
rm(list = c(ls(pattern = "^afb"), "lf", "map"))
