...
# Convert to lat/lon if shp uses projected CRS and lat/lon columns exist

afbSF <- if(!st_is_longlat(afbSF) && "lat" %in% names(afbSF) && "lon" %in% names(afbSF)) {
  # Transform geometry from projected CRS to lat/lon
  afbSF <- st_transform(afbSF, crs = 4326)  # 4326 is WGS84 (lat/lon)
}

afbSF <- if (xmax(rx) == 360 & xmax(afb) < 0) {st_shift_longitude(afbSF)} else afbSF # adjustment in case ncdf lon scale is 0-360

elev <- afbSF$Elev_Mean
